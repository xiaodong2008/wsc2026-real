...
function render() { favCount.textContent = playlist.querySelectorAll('.track--fav').length; }
function save() {
...
try {
  ...
  tracks().forEach((li) => { if (saved.includes(keyOf(li))) li.classList.add('track--fav'); });
} catch {}
render();
```
