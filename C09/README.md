...
模板：Express，但 `server.js` 是纯 Node `http`，不能 `require('express')`。

校验顺序不能换：房间不存在 404，容量 422，开放时间 422，结束时间不晚于开始 422，冲突 409，成功 201。首尾相接不算冲突，重叠判断用严格小于。新 id 是现有最大 id 加 1。错误字段名比赛当天以 starter 的 404 为准，这里用 `error`。
