...
模板：Vanilla。整页在 `index.html`。

每个字母按自己到光标的距离单独变粗，而且要连续，不能一档一档跳。这里同时写了 `font-variation-settings` 和 `-webkit-text-stroke-width`。比赛当天如果 media 里有可变字体，用那个字体；没有就靠 stroke。

本地打开 `index.html`，移动鼠标。
