...
模板：Vanilla。交卷文件是 `result/result.sql`。

月份过滤写在 `screenings` 上。取消的预订排除。收入小于等于 500 的电影排除，所以是 `> 500`。并列时再按 `movie_title` 升序。

`media/schema.sql` 是按题目假设的练习库，不是比赛原文件。现场先对一下列名。取消字段可能是 `status`、`is_cancelled` 或 `cancelled_at`。
