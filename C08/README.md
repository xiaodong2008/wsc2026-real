...
模板：Express，但 `server.js` 是纯 Node `http`，不能 `require('express')`。模板的 Dockerfile 仍然会执行 `node server.js`。

`total` 是过滤之后的数量。没有匹配时 `total_pages` 也是 1。页码超出范围返回 200 和空数组。`search` 匹配 title 或 company，不区分大小写，是子串。
