...
模板：Laravel。媒体数据进 MySQL，再用 HTML table 显示。

表名是 `starter03_records`（共用库，必须带自己的前缀）。迁移写入的 8 行和 `database/data/records.csv` 相同。比赛当天先看媒体文件是 CSV、SQL 还是 JSON，优先用 phpMyAdmin 导入，再把查询里的表名改成实际表。
