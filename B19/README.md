...
模板：Vanilla。只改 `script.js` 标记以下。

拼块的 `background-position` 是 `-${targetX}px -${HOLE_Y}px`。`targetX` 从 100 到 540。拖拽用 Pointer Events，并 `setPointerCapture`。松手时误差不超过 5px 就吸到缺口，加上 `solved`，1.5 秒后换新题。误差之外停在原地。
