...
模板：Vanilla。只改 `js/script.js` 标记以下。

用 `sessionStorage`，不要写成 `localStorage`。`playlist`、`favCount`、`STORAGE_KEY` 上面已经声明，不要再声明一次。恢复之后要再 `render()` 一次。

本地打开 `index.html`。点爱心，刷新，计数还在；关掉标签页再开就没了。
